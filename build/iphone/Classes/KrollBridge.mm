...
NSString * LessiusServicesScreencast$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation LessiusServicesScreencastObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_lessiusservicesscreencast);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
-(void)injectPatches
{
	// called to inject any LessiusServicesScreencast patches in JS before a context is loaded... nice for 
...
{
	// create LessiusServicesScreencast global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_lessiusservicesscreencast = [[LessiusServicesScreencastObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_lessiusservicesscreencast];
	
	NSString *_lessiusservicesscreencastNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _lessiusservicesscreencastNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_lessiusservicesscreencast valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:LessiusServicesScreencast$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.lessiusservicesscreencast.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.lessiusservicesscreencast.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
